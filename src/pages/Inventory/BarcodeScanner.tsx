...
export default function BarcodeScanner() {
  ...
  const handleSearch = useCallback(async (code: string) => {
    ...
    setScanResult(null);
    try {
      const data = await apiClient.get(`/inventory/barcode/${encodeURIComponent(trimmed)}`);
      setScanResult(data);
    } catch (err: any) {
      setError(err.message?.includes("404") || err.message?.includes("ونه موندل شو")
        ? `جنس ونه موندل شو. کوډ "${trimmed}" د سیستم کې نشته.`
        : `خطا: ${err.message}`);
    } finally {
      setLoading(false);
    }
  ...
  const startCamera = useCallback(async () => {
    ...
    if (!("BarcodeDetector" in window)) {
      setCameraError("ستاسې براوزر د کیمرې سکین ملاتړ نه کوي. مهرباني وکړئ لاندې لارو پیل کوډ دننه کړئ.");
      return;
    }
    try {
      ...
      detectorRef.current = new (window as any).BarcodeDetector({ formats: ["qr_code", "code_128", "code_39", "ean_13", "ean_8"] });
      ...
      if (err.name === "NotAllowedError") {
        ...
        setCameraError("کیمره پرانیستل ونشول. لاندې لارو پیل کوډ دننه کړئ.");
      }
    }
  ...
  if (!canAccess) {
    return (
      <div className="rounded-2xl border border-red-200 bg-red-50 dark:border-red-900/30 dark:bg-red-900/10 p-8 text-center" dir="rtl">
        ...
        <p className="text-red-600 dark:text-red-300 mt-2 text-sm">تاسې د دې پاڼې لیدلو صلاحیت نه لرئ. یوازې سوپر اډمین او اډمین کولی شي د بارکوډ سکینر ته لاسرسی ولري.</p>
            ...
            <div className="space-y-3">
              ...
              {cameraActive ? (
                <div className="space-y-2">
                  <video ref={videoRef} autoPlay playsInline className="w-full rounded-lg border border-gray-300 dark:border-gray-700" style={{ maxHeight: 200 }} />
