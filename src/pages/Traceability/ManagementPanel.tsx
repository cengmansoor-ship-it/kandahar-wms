    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {faculties.map((f, i) => (
            <div key={f.id} className="flex items-center justify-between bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 shadow-sm hover:shadow-md transition-all animate-slide-up" style={{ animationDelay: `${i * 30}ms` }}>
              <div className="flex items-center gap-3">
                <button onClick={() => doDelete(f.id)} className="p-1.5 rounded-lg bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/40 text-red-500 text-sm transition-all" title="حذف">🗑️</button>
              ...
              <div className="text-right">
                <p className="font-semibold text-gray-800 dark:text-white text-sm">{f.name_ps} / {f.name_fa}</p>
                {f.level && <span className="text-xs bg-sky-100 text-sky-700 dark:bg-sky-900/30 dark:text-sky-300 px-2 py-0.5 rounded-full">{LEVEL_LABELS[f.level] || f.level}</span>}
              </div>
            </div>
          ))}
        </div>
